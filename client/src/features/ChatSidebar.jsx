import BASE_URL, { WBS_URL } from '../utils/config';
...
const ChatSidebar = () => {
    ...
    const fetchGameName = async () => {
        try {
            const response = await fetch(
                `${BASE_URL}get-game-name?gameId=${gameId}`,
                {
                    method: 'GET',
                    credentials: 'include',
                },
    ...
    const fetchGameMessages = async () => {
        ...
        const data = await response.json();
        if (response.ok) setGameChatMessages(data.messages);
    };
    ...
    useEffect(() => {
        ...
        wsRef.current.onopen = () => {
            ...
            if (isGameRoute && gameId) {
                wsRef.current.send(
                    JSON.stringify({ type: 'gameChat', gameId }),
                );
            }
        };
                        ...
                        {selectedFriend.current ? (
                            (
                                ...
                                friends
                                    .find(
                                        (f) =>
                                            f.userId === selectedFriend.current,
                                    )
